...
final class ComposerScripts
{
    ...
    {
        if (! is_dir($directory)) {
            echo \sprintf("\033[43m WARN \033[0m Cannot recursively delete \"%s\" as it does not exist.\n", $directory);
    ...
    private static function updateVscodeIntelephenseEnvironmentIncludePaths(): void
    {
        ...
        if (false === $contents) {
            echo \sprintf("\033[41m FAIL \033[0m Cannot get the contents of %s as it is probably missing or unreadable.\n", self::VSCODE_SETTINGS_JSON);
        ...
        try {
            ...
            if (file_put_contents(self::VSCODE_SETTINGS_JSON, $newContents) === false) {
                echo "\033[41m FAIL \033[0m Cannot save the new contents to .vscode/settings.json.\n";
