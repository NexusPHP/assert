...
namespace Nexus\Assert\Tests\TypeInference\IsSubclassOfExpectation;

use Nexus\Assert\Assert;

use function PHPStan\Testing\assertType;

function test_is_subclass_of_with_string(mixed $a, mixed $b, mixed $c, string $className): void
{
    $assert1 = Assert::that($a)->isSubclassOf($className);
    assertType('Nexus\\Assert\\Expectation<class-string|object>', $assert1);
    assertType('class-string|object', $a);

    $assert2 = Assert::that($b)->not()->isSubclassOf($className);
    assertType('Nexus\\Assert\\NegatedExpectation<mixed>', $assert2);
    assertType('mixed', $b);

    $assert3 = Assert::that($c)->nullOr()->isSubclassOf($className);
    assertType('Nexus\\Assert\\NullableExpectation<class-string|object|null>', $assert3);
    assertType('class-string|object|null', $c);
}

function test_is_subclass_of_with_class_constant(mixed $a, mixed $b, mixed $c): void
{
    $assert1 = Assert::that($a)->isSubclassOf(\DateTimeInterface::class);
    ...
    $assert2 = Assert::that($b)->not()->isSubclassOf(\DateTimeInterface::class);
    assertType('Nexus\\Assert\\NegatedExpectation<mixed>', $assert2);
    assertType('mixed', $b);

    $assert3 = Assert::that($c)->nullOr()->isSubclassOf(\DateTimeInterface::class);
 ...
 */
function test_is_subclass_of_with_generic_class_string(mixed $a, string $className): void
{
    $assert = Assert::that($a)->isSubclassOf($className);
 ...
 */
function test_is_subclass_of_with_union_generic_class_string(mixed $a, string $className): void
{
    $assert = Assert::that($a)->isSubclassOf($className);
...
function test_is_subclass_of_with_object(mixed $a, \DateTimeInterface $class): void
{
    $assert = Assert::that($a)->isSubclassOf($class);
